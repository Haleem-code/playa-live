...
import { Upload, Calendar, Gamepad, Loader2, Radio, Clock, Timer } from 'lucide-react';
...
export default function CreateStreamPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const durationMinutes = showCustomDuration && formData.customDuration 
      ...
      const bettingDeadline = new Date(startDate.getTime() + durationMinutes * 60 * 1000).toISOString();
      ...
      const errorMessage = error.response?.data?.message || error.message || 'Failed to create stream';
  ...
  if (!user) {
    return (
      <div className="max-w-3xl mx-auto text-center py-12">
        <p className="text-slate-400">You must be logged in to create a stream</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <h1 className="text-2xl font-bold mb-6">Create New Stream</h1>
      ...
      <div className="card">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Title */}
          <div>
            <label className="block text-sm font-medium text-slate-400 mb-2">Stream Title</label>
            <input 
              type="text" 
              className="input" 
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-400 mb-2">Player 1 (You)</label>
            ...
            {/* Player 2 (Search) */}
            <div>
              <label className="block text-sm font-medium text-slate-400 mb-2">Select Your Second Player</label>
              <UserSearchDropdown
                onSelect={(player2) => setFormData(prev => ({ ...prev, player2 }))}
          ...
          {/* Game Category */}
          <div>
            <label className="block text-sm font-medium text-slate-400 mb-2">Game / Category</label>
            <div className="relative">
              <Gamepad className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-500" />
              <select 
                className="input pl-10 appearance-none"
                value={formData.gameCategory}
                onChange={(e) => setFormData(prev => ({ ...prev, gameCategory: e.target.value }))}
              >
          ...
          <div>
                ...
                <button
                  ...
                  onClick={() => {
                    if (option.value === -1) {
                      ...
                      setFormData(prev => ({ ...prev, duration: option.value, customDuration: '' }));
                    }
                  }}
                  className={`p-3 rounded-lg border-2 transition-all text-sm font-medium ${
                    (option.value === -1 && showCustomDuration) || (!showCustomDuration && formData.duration === option.value)
            ...
            {showCustomDuration && (
              <div className="relative">
                ...
                />
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 text-sm">minutes</span>
              </div>
            )}
            
            <p className="text-xs text-slate-500 mt-2">
              Selected: {showCustomDuration && formData.customDuration 
                ? `${formData.customDuration} minutes` 
                : formData.duration === 30 ? '30 minutes' 
                : formData.duration === 60 ? '1 hour' 
                : '2 hours'
              }
            </p>
          </div>
          ...
          {/* Schedule */}
          <div>
            <label className="block text-sm font-medium text-slate-400 mb-2">Stream Status</label>
            <div className="grid grid-cols-2 gap-3 mb-4">
              <button
                type="button"
                onClick={() => setFormData(prev => ({ ...prev, streamStatus: 'scheduled' }))}
              ...
              <button
                type="button"
                onClick={() => setFormData(prev => ({ ...prev, streamStatus: 'live', startTime: '' }))}
                className={`flex items-center justify-center gap-2 p-4 rounded-lg border-2 transition-all ${
            ...
            {formData.streamStatus === 'scheduled' && (
              <>
                ...
                <p className="text-xs text-slate-500 mt-1">Leave blank to allow starting anytime. You&apos;ll click &quot;Go Live&quot; when ready.</p>
              </>
            )}
          ...
          <div>
            ...
            >
              {uploadingThumbnail ? (
                <>
                  {thumbnailPreview && (
                    <img src={thumbnailPreview} alt="Image preview" className="w-full max-h-48 object-contain rounded mb-2 opacity-50" />
                  )}
                ...
                <>
                  <img 
                    ...
                    onError={(e) => {
                      ...
                      if (thumbnailPreview && e.currentTarget.src !== thumbnailPreview) {
                        e.currentTarget.src = thumbnailPreview;
                      }
                    }}
                  />
                  <p className="text-sm text-green-400">{formData.thumbnailUrl ? 'Image uploaded' : 'Click to change'}</p>
                </>
              ) : (
                <>
                  <Upload className="w-10 h-10 text-slate-500 mx-auto mb-2" />
                  <p className="text-sm text-slate-400">Click to upload or drag and drop</p>
                  <p className="text-xs text-slate-600 mt-1">JPEG, PNG, GIF or WebP (max. 5MB)</p>
                </>
              )}
            </label>
          </div>
