...
export function StreamCard({ stream }: StreamCardProps) {
  ...
  const displayUrl = stream.thumbnail_url;
  ...
  const scheduledDate = stream.start_time
    ? new Date(stream.start_time)
    : new Date(stream.betting_deadline);
  ...
  const statusColor =
    statusColors[stream.status.toLowerCase()] || "bg-slate-500";
      ...
      <div className="relative border border-white/5 bg-black/20 backdrop-blur-sm p-0 h-full flex flex-col">
        ...
        {isScheduled && (
          <div className="absolute inset-0 z-20 bg-black/80 backdrop-blur-sm flex flex-col items-center justify-center pointer-events-none border-b border-white/10">
            ...
            <span className="text-xs text-zinc-500 mb-2 font-mono uppercase tracking-widest">
              Starts In
            </span>
            {countdown ? (
        ...
        {/* Thumbnail */}
        <div className="relative aspect-video bg-zinc-900 border-b border-white/5 group-hover:border-blue-500/30 transition-colors">
          {!imageError && displayUrl ? (
            ...
            <div className="flex items-center justify-center h-full flex-col text-zinc-800 gap-3 bg-zinc-900 pattern-grid-lg">
          ...
          <div
            className={`absolute top-0 right-0 ${statusColor} px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-white shadow-sm`}
          >
            {stream.status}
          </div>
          ...
          <div className="flex items-center justify-between text-xs text-zinc-500 pt-3 border-t border-white/5 font-mono">
            <div className="flex items-center gap-2">
              <span className="text-zinc-300 font-bold">
                {stream.stats?.total_pool_sol?.toLocaleString() ?? 0} SOL
              </span>
