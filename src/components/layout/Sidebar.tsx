      ...
      <div className="py-2">
        ...
        <div className="space-y-0.5">
          {FOLLOWED_CHANNELS.map((channel, i) => (
            <div
              key={i}
              className="group flex items-center justify-between px-3 py-1.5 hover:bg-[#26262c] cursor-pointer mx-2 rounded-md transition-colors"
            >
              <div className="flex items-center gap-3 overflow-hidden">
                <div
                  className={`w-8 h-8 rounded-full ${channel.avatar} flex items-center justify-center text-[10px] font-bold text-white shrink-0`}
                >
                  {channel.name[0]}
                </div>
                <div className="flex flex-col overflow-hidden">
                  <span className="text-sm font-semibold text-zinc-200 truncate group-hover:text-white">
                    {channel.name}
                  </span>
