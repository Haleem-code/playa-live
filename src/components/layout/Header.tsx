...
import { WalletTransferModal, ViewType } from "./WalletTransferModal";
...
export function Header({ onMenuClick }: { onMenuClick?: () => void }) {
  ...
  const [modalView, setModalView] = useState<ViewType>("selection");
    ...
    const fetchData = async () => {
      try {
        ...
        if (balanceResponse.success) {
          const walletBalance =
            balanceResponse.data?.balance || balanceResponse.balance || 0;
        ...
        <div className="flex items-center gap-4 lg:w-64">
          ...
          <Link href="/app" className="hidden lg:flex items-center gap-2 group">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-xl tracking-tight text-transparent bg-clip-text bg-white">
                Playa
              </span>
            </div>
          </Link>
              ...
              <div className="fixed inset-x-4 top-20 md:absolute md:inset-auto md:right-0 md:top-full md:mt-2 md:w-64 bg-[#18181b] border border-white/10 rounded-xl shadow-2xl py-2 z-50 md:rounded-lg ring-1 ring-black/5">
                ...
                <div className="p-2 grid grid-cols-2 gap-2">
                  <button
                    ...
                    className="flex flex-col items-center justify-center p-3 cursor-pointer text-sm text-white bg-gradient-to-br from-emerald-500/10 to-emerald-600/5 border border-emerald-500/20 hover:border-emerald-500/40 hover:bg-emerald-500/15 rounded-lg transition-all"
                  >
                    <span className="text-emerald-400 text-lg mb-0.5">↓</span>
                  ...
                  <button
                    ...
                    className="flex flex-col items-center justify-center p-3 cursor-pointer text-sm text-white bg-gradient-to-br from-blue-500/10 to-blue-600/5 border border-blue-500/20 hover:border-blue-500/40 hover:bg-blue-500/15 rounded-lg transition-all"
                  >
                    <span className="text-blue-400 text-lg mb-0.5">↑</span>
