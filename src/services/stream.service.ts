...
function normalizeStream(stream: any): Stream {
  return {
    ...
    thumbnail_url: fixImageUrl(
      stream.thumbnail_url || stream.thumbnailUrl || stream.coverImage,
    ),
    ...
    live_started_at:
      stream.live_started_at || stream.liveStartedAt || undefined,
    status:
      stream.status || (stream.is_live || stream.isLive ? "live" : "scheduled"),
    is_live: stream.is_live ?? stream.isLive ?? false,
    stats: {
      current_viewers:
        stream.stats?.current_viewers || stream.stats?.currentViewers || 0,
      total_pool_sol:
        stream.stats?.total_pool_sol || stream.stats?.totalPoolSol || 0,
      player1_bets_sol:
        stream.stats?.player1_bets_sol || stream.stats?.player1BetsSol || 0,
      player2_bets_sol:
        stream.stats?.player2_bets_sol || stream.stats?.player2BetsSol || 0,
      player1_bet_count:
        stream.stats?.player1_bet_count || stream.stats?.player1BetCount || 0,
      player2_bet_count:
        stream.stats?.player2_bet_count || stream.stats?.player2BetCount || 0,
    },
    livekitRoomName:
      stream.livekitRoomName || stream.livekit_room_name || undefined,
    player1_walletAddress:
      stream.player1_walletAddress ||
      stream.player1WalletAddress ||
      stream.player1_wallet_address,
    player2_walletAddress:
      stream.player2_walletAddress ||
      stream.player2WalletAddress ||
      stream.player2_wallet_address,
  };
}
...
class StreamService {
  async getLiveStreams(): Promise<Stream[]> {
    try {
      const response =
        await apiClient.get<ApiResponse<{ streams: Stream[]; count: number }>>(
          "/streams/live",
        );
  ...
  async getScheduledStreams(): Promise<Stream[]> {
    try {
      const response = await apiClient.get<
        ApiResponse<{ streams: Stream[]; total: number }>
      >("/streams", {
  ...
  async getStreamById(streamId: string): Promise<Stream | null> {
    try {
      const response = await apiClient.get<ApiResponse<{ stream: Stream }>>(
        `/streams/${streamId}`,
      );
      const body = response.data;
  ...
  async createStream(streamData: CreateStreamData): Promise<Stream | null> {
    try {
      ...
      const isValidUrl =
        ...
        (streamData.thumbnailUrl.startsWith("http://") ||
          streamData.thumbnailUrl.startsWith("https://"));

      // Validate wallet addresses - both are REQUIRED for betting pool initialization
      if (
        !streamData.player1WalletAddress ||
        !streamData.player2WalletAddress
      ) {
        console.error("Both wallet addresses are required for stream creation");
        throw new Error(
          "Both player wallet addresses are required to create a stream",
        );
      }
      ...
      const response = await apiClient.post<
        ApiResponse<{ stream: Stream; transaction: string }>
      >("/streams/create", backendData);
      const body = response.data;
      ...
      if (streamData.isLive && resultAny.id) {
        ...
        try {
          const statusResponse = await apiClient.patch<
            ApiResponse<{ stream: Stream }>
          >(`/streams/${resultAny.id}/status`, { status: "live" });
          console.log("Status update response:", statusResponse.data);

          if (
            statusResponse.data?.success &&
            statusResponse.data?.data?.stream
          ) {
          ...
          // Failed to update status, but stream created
        }
      }
  ...
  async getAllStreams(filters?: {
    ...
    try {
      ...
      if (filters?.gameCategory)
        queryParams.gameCategory = filters.gameCategory;
  ...
  async uploadCoverImage(
    streamId: string,
    file: File,
  ): Promise<ApiResponse<{ coverImage: string; stream: Stream }>> {
    try {
      const formData = new FormData();
      formData.append("coverImage", file);

      const response = await apiClient.post<
        ApiResponse<{ coverImage: string; stream: Stream }>
      >(`/streams/${streamId}/cover-image`, formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });

      // Fix image URL in response
      if (response.data.success && response.data.data?.coverImage) {
        response.data.data.coverImage =
          fixImageUrl(response.data.data.coverImage) || "";
      }
      if (response.data.success && response.data.data?.stream?.coverImage) {
        response.data.data.stream.coverImage = fixImageUrl(
          response.data.data.stream.coverImage,
        );
      }
